...
{ // p is denominator in division and q is numerator t1 is t entering and t2 is t leaving 
double t=q/p; 
...
// inside test wrt left edge t for the left edge is t=(xmin-x0) /dx .... order of numerator is reversed and you get -dx 
if(cliptest(dx,xmax-x0,&te,&tl))
// inside test wrt right edge t for the right edge is t=(xmax-x0)/dx.... first parameter becomes q and second p then t=q/p 
if(cliptest(-dy,y0-ymin,&te,&tl))
// inside test wrt bottom edge same story numerator by demoninator to find 
if(cliptest(dy,ymax-y0,&te,&tl))
// inside test wrt top edge in one way cohen sutherland and liang barasky are similar 
{ 
if(tl<1.0) 
{ 
x1=x0+tl*dx; 
...
//liangbaraskylineclipanddraw(10,20,200,190); 
glFlush(); 
} 
void myinit() 
